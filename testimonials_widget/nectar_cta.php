...
if($btn_style == 'material') {
  ...
  echo '<'.$heading_tag.'> <span class="text">'.$text.' </span>' . '<br>' . '<div id="phone-footer">' . '1-877-766-5877' . '</div>';
  ...
  echo '<div class="nectar-cta '.$class.'" data-style="'.$btn_style.'" data-alignment="'.$alignment.'"  data-text-color="'.$text_color.'" >';
  echo '<'.$heading_tag. $style.'> <span class="text">'.$text.' </span>' . '<br>' . '<div id="phone-footer">' . '1-877-766-5877' . '</div>';
  echo  '<span class="link_wrap"><a '.$target .' class="link_text" href="'.$url.'">'.$link_text.'<span class="arrow"></span></a></span>'; 
  echo '</'.$heading_tag.'></div>';
}

echo '<script>document.getElementById("phone").innerHTML = ' . '<span class="phone-span">' . $phoneField  . '</span>;' . '</script>'
?>
33472